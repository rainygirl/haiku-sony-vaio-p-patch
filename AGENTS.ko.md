# VAIO P Haiku OS 패치 — 개발 노트

English version: [`AGENTS.md`](AGENTS.md).

설치와 빌드 방법은 [`README.ko.md`](README.ko.md)에 있습니다. 이 문서는 그 외 전부입니다 — 각 패치가 무엇을 왜 고치는지, 추측이 아니라 실제로 측정한 것이 무엇인지, 그리고 처음에 시간을 잡아먹었던 함정들입니다.
...
- **두 번째 논리 CPU, 그리고 부팅 견고성** — `smp.cpp`/`smp_trampoline.S`(부트로더): 이 개체의 HT sibling이 기동하지 않았고, 업스트림의 대기 루프에는 타임아웃이 아예 없어서 `smp_boot_other_cpus()`가 그 자리에서 영원히 멈췄고 결국 부팅 자체가 되지 않았습니다. 이제 모든 대기에 상한을 두고, AP마다 INIT/SIPI/SIPI 전체 시퀀스(트램폴린 스택 재설정 포함)를 최대 3회까지 재시도한 뒤 포기하고 응답한 CPU 수만으로 부팅을 계속합니다. 그리고 이제 AP가 실제로 기동합니다 — Linux처럼 BIOS warm-reset vector를 설정하고, gdt 로드가 `di`가 0이라는 가정을 하지 않으며, STARTUP IPI가 SDM이 요구하는 level 플래그를 실어 보냅니다. 그리고 AP를 `_start()`에서 미리 깨웁니다 — 이 기기가 AP를 기동시킬 수 있는 창이 전원 인가 후 몇 초뿐이고 통상적인 기동 시점은 그 밖이기 때문입니다. **이 기기에서 두 CPU가 모두 돕니다** — 아래 "두 번째 논리 CPU" 항목을 보세요. 상한 있는 재시도는 대비책으로 남습니다. `evglock.c`: 핸들러 설치 도중 Global Lock SCI가 동기적으로 발생할 때 필요한 락이 아직 없어서 터지는 ACPICA 레이스를 수정. `pci.cpp`: 정렬 안 된 32비트 config 공간 접근을 그냥 거부하지 않고 16비트 접근 두 번으로 쪼갬. `acpi_irq_routing_table.cpp`: 잘못됐을 수 있는 ACPI 서술자를 그대로 믿는 대신 IRQ 극성/트리거 모드를 고정값으로 보정. `vfs_boot.cpp`: 부팅 파티션 탐색을 즉시 패닉하지 않고 최대 60초까지 재시도 — 열거가 느린 USB 부팅 매체에 더해, 부팅 초기 USB halt/복구 사이클(각각의 안정화 지연, 포트 파워사이클, 쿨다운 재무장이 겹겹이 쌓임)이 부팅 장치의 열거를 이전 버전의 10초 한도 너머로 밀어내면서 실기에서 "did not find any boot partitions" 패닉으로 나타났습니다.
...
**이 기기가 AP를 기동시킬 수 있는 시간 창은 전원 인가 후 몇 초뿐입니다.** 직접 측정했습니다 — 부트로더에서 SIPI 전에 1초 지연을 넣으면 여전히 동작하고, 5초를 넣으면 실패합니다. 그 창을 넘기면 AP는 INIT과 SIPI에 아예 반응하지 않습니다. 아무것도 실행하지 않고, 오류도 보고하지 않으며, 10회를 재시도해도 같습니다.

업스트림은 부트로더가 커널·애드온과 (Haiku가 기본으로 켜는 `load_symbols`가 있으면) 그 심볼들까지 전부 디스크에서 읽은 **뒤에** SIPI를 보냅니다. 이 기기에서는 그 시점이 창 밖이고, 그래서 AP가 기동하지 않은 채 CPU 하나로 조용히 올라옵니다.

이게 원인의 전부입니다. 실패하는 상태에서 부트로더가 AP에게 넘기는 것은 전부 정상임을 확인했습니다 — 트램폴린 코드 체크섬 전후 동일, gdt 디스크립터 `0017/0009e008`, gdt 코드 엔트리 `0000ffff:00cf9a00`, `cr3`, 최종 `esp`, 로컬 APIC 매핑·소프트웨어 활성화·정상 id와 버전 레지스터, 트램폴린 두 페이지 모두 identity 매핑, 그리고 ICR이 `icr2 01000000`(목적지 1) / `icr1 0000069f`(벡터 0x9f, delivery mode 6)로 정확히 프로그램됨. 손상된 것은 없었고, AP를 더 이상 깨울 수 없었을 뿐입니다.

### 수정

`smp_wake_other_cpus_early()`를 `_start()`의 `smp_init()` 직후에 호출해, 창이 열려 있는 동안 INIT/SIPI를 보냅니다. 그렇게 일찍 깨우는 데 필요한 것은 gdt뿐이고 그 내용은 정적이라 커널이 존재하기 전에도 가능합니다. AP는 32비트 보호 모드까지 도달해 그 사실을 기록한 뒤, 트램폴린에서 페이지 디렉터리 슬롯이 채워지기를 기다리며 대기합니다. `smp_boot_other_cpus()`는 예전과 똑같이 그 슬롯을 채우고, AP는 그대로 커널로 진입합니다.

AP가 우리 코드를 실행하기 시작하면, 펌웨어가 유휴 sibling에게 무엇을 하든 더는 문제가 되지 않습니다. early wake가 실패한 경우 `smp_boot_other_cpus()`가 기존 INIT/SIPI 전체 시퀀스를 그대로 수행하므로, 이 경로에 의존하는 부분은 없습니다.
...
이 절의 내용은 사흘 동안 틀린 상태였고, 그 이유는 남겨둘 가치가 있습니다.

- **진단이 `0x9d000`에 썼습니다.** 이 주소는 `mmu.cpp`의 `kPageTableRegionEnd`(`0x9e000`) 아래, 즉 부트로더가 페이지 테이블로 넘겨주는 영역입니다. 그래서 모든 진행 마커가 읽히기 전에 덮어써졌습니다. 그 근거로 "AP가 명령 하나도 실행하지 않는다", "펌웨어가 스레드를 비활성화했고 MADT만 갱신 안 됐다"는 결론을 냈습니다. 부트로더 진단용 스크래치는 쓰이지 않음이 증명된 곳에서 가져와야 합니다 — 트램폴린 스택 페이지 중간(`0x9e800`)이 적합합니다.
- **SIPI 전에 출력하면 결과가 바뀝니다.** 부트로더의 `dprintf()`는 syslog 버퍼와 시리얼에 쓰는데, 그 작업량만으로 기동 시점이 창 밖으로 밀립니다. "이 빌드는 되고 저 빌드는 안 된다"는 비교 여러 건이 실은 계측 장치를 측정하고 있었습니다. SIPI 전에는 지역 변수로만 수집하고 출력은 대기 이후에 해야 합니다.
- **빌드마다 부팅 1회씩만 표본을 잡았습니다.** 성공을 마음대로 재현할 수단이 없는 상태에서 각 빌드에 한 번씩만 부팅하고 그 차이를 빌드 차이로 읽었습니다. 아니었습니다 — 같은 바이너리가 성공하기도 실패하기도 했습니다. gdt 로드의 바이트 오프셋 임계값, CMOS I/O 지연, 콜드/웜 부팅, CMOS 오염, 리셋을 넘어 남는 AP 상태 — 그 위에 세운 가설 전부가 틀렸습니다.

전환점은 **갓 구운 USB가 첫 부팅에서는 되고 두 번째 부팅부터 안 된다**는 관찰이었고, 거기서 처음으로 통제 가능한 변수가 나왔습니다. 그것이 `load_symbols`로 이어졌고, 그 설정을 뒤집으면 성공과 실패를 마음대로 만들 수 있게 되면서 비로소 실제 측정(SIPI 전 지연)이 가능해졌습니다.

그 과정에서 만든 세 가지 변경은 **필요 없음이 검증되어** 패치에 들어가지 않습니다 — gdt 로드 전 `di`를 0으로 강제, Linux 방식의 BIOS warm-reset vector 설정, STARTUP IPI의 level 플래그 교정. 타이밍만 고치면 **전혀 수정하지 않은 부트로더로도** AP가 올라옵니다. 이 중 둘은 업스트림 코드의 실제 결함을 짚고 있지만(아래), 이 기기를 망가뜨린 원인은 아니었습니다.

### 작업 중 발견한 업스트림의 이상한 점

타이밍이 맞으면 이 기기에는 영향이 없습니다.

- `smp_trampoline.S`는 gdt 로드를 `.byte 0x66, 0x0f, 0x01, 0x15, 0x00, 0xe0, 0x09, 0x00`으로 손수 인코딩하고 주석에 `lgdt 0x9e000`이라 적어뒀습니다. 그렇게 어셈블되지 않습니다 — 16비트 주소 모드에서 modrm `0x15`는 `[DI]`이므로 디스크립터를 `ds:di`에서 읽고, `0x9e000`에 떨어지는 건 `di`가 우연히 0이기 때문이며, 주소 4바이트는 명령으로 실행됩니다(`add %ah,%al`, 이어서 `or %ax,(%bx,%si)` — 방금 로드한 디스크립터에 씁니다). 다만 주소 계산은 `ds` 상대로 남겨야 합니다. real mode라 절대 형식은 `0xffff` 세그먼트 한계를 넘어 폴트합니다.
- STARTUP IPI가 level 플래그 0으로 나갑니다. 마스크 `0xfff0f800`이 직전 level 트리거 INIT de-assert의 bits 15:11을 물려받기 때문입니다. SDM은 INIT level de-assert를 제외한 모든 delivery mode에서 이 플래그를 세우도록 요구합니다.

### 여전히 유효한 부정 결과

| 가설 | 결과 |
| --- | --- |
| SIPI가 아예 안 나간다 (MADT에 APIC 버전 필드가 없음) | ACPI 경로가 `cpu_apic_version = 0x10`을 하드코딩해 SIPI가 2회 나갑니다. |
| safemode 설정으로 SMP가 꺼져 있다 | 이 기기의 커널 설정에서 `#disable_smp true`는 주석 처리 상태. |
| 이 패치가 ACPI를 껐다 | 아닙니다 — ACPICA가 Sony SSDT를 로드하고, `intel_est`가 ACPI EST를 쓰며, 로더가 `using ACPI to detect MP configuration`을 출력합니다. |
| 트램폴린이 BIOS의 EBDA에 들어간다 | BIOS가 재래식 메모리 639 KB, EBDA를 `0x9fc00`으로 보고합니다. `0x9f000`의 트램폴린은 `0xc00` 바이트보다 훨씬 작습니다. |
| MADT의 sibling APIC ID가 틀렸다 | APIC ID를 쓰지 않는 "all excluding self" shorthand로 보내도 동일했습니다. |
| 펌웨어의 다른 토폴로지 서술이 어긋난다 | 그런 게 없습니다 — MP 테이블 영역 두 곳을 강제 스캔했으나 `_MP_` 서명이 없습니다. |

증거처럼 보이지만 아닌 것 둘. `APIC_ERROR_STATUS`는 모든 INIT·SIPI 뒤에 `0x00`이지만, SDM은 send/receive accept error를 P6 family와 Pentium에 한정하며 이 CPU는 세우지 않습니다. ICR의 delivery-status 비트가 내려가는 것도 로컬 APIC가 **송신을 마쳤다**는 뜻일 뿐입니다.

이 CPU가 자신에 대해 보고하는 값:
...
`CPUID.1 EBX[23:16]`은 패키지가 **주소지정 가능한** 논리 프로세서 수이고, 하이퍼스레딩 활성화 여부와 무관하게 2로 읽히는 설계 상수입니다. **활성화된** 개수를 알 수 있는 유일한 CPUID 수단인 leaf `0xb`는 이 CPU에 없고 `MSR_CORE_THREAD_COUNT`도 없습니다.

**AAE31** 관련: erratum은 이 스테핑에 해당하지만, 이는 이 CPU에서 **cpuidle C-state 경로를 포기한 이유**이지 하이퍼스레딩이 위험하다는 근거가 아닙니다. 이후 두 스레드로 85분간 2-way 병렬 컴파일을 돌렸고 행·크래시·컴파일 실패 없이 처리량 약 +39%였습니다.

마지막으로 **`smp_trampoline.S`는 `0x9e000`/`0x9f000`을 하드코딩합니다** — 스택 세그먼트, gdt 주소, `ljmp` 목표, 초기 `esp` 전부입니다. `smp.cpp`의 `trampolineCode`/`trampolineStack`만 바꿔도 효과가 없고, PXE 플랫폼이 이 파일 사본을 따로 두는 이유가 그것입니다.
...
**이 패치로 빌드한 패키지를 설치하기 전에 반드시 되돌리세요.** 차단 목록은 버전이 아니라 경로를 숨기므로, 원래 겨냥했던 고장난 `usb_webcam.media_addon`뿐 아니라 새 `haiku.hpkg`에 들어 있는 **수정된** 애드온까지 똑같이 숨겨버립니다. 그리고 그 자리를 낡은 non-packaged 사본이 대신 차지합니다. 이 패치는 usb_webcam을 소스에서 고치므로(`UVCCamDevice`, `UVCDeframer`, `CamDevice` — 위 "웹캠 (UVC)" 항목 참고), 정작 써야 할 것은 패키지 안의 애드온입니다:
...
차단 목록은 이미지를 다시 빌드하지 않고 애드온 수정을 시험해 보기 위한 수단입니다. 수정이 패키지 안으로 들어간 뒤에는 남겨둘 이유가 없습니다.
